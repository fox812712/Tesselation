...
void Cad::CEdge2D::GetBoundingBox(double& x_min, double& x_max, double& y_min, double& y_max) const
{
	...
	else if (itype == 3){//  RCJ 6/14/2017 
		
		const unsigned int n = aRelCoSpline.size() / 2;
		po_e.x < po_s.x ? x_min = po_e.x, x_max = po_s.x : x_min = po_s.x, x_max = po_e.x;
		po_e.y < po_s.y ? y_min = po_e.y, y_max = po_s.y : y_min = po_s.y, y_max = po_e.y;
		for (unsigned int i = 0; i < n; i++){
			Com::CVector2D po0(aRelCoSpline[i*2],aRelCoSpline[i*2+1]);
			x_min = (po0.x < x_min) ? po0.x : x_min;
			x_max = (po0.x > x_max) ? po0.x : x_max;
			y_min = (po0.y < y_min) ? po0.y : y_min;
			y_max = (po0.y > y_max) ? po0.y : y_max;
...
Com::CVector2D Cad::CEdge2D::GetTangentEdge(bool is_s) const {
	...
	else if (this->itype == 3){//  RCJ 6/14/2017 

		const std::vector<double>& reclomsh = aRelCoSpline;

		if (reclomsh.size() == 0) return v;
		const unsigned int ndiv = reclomsh.size() / 2;
		if (is_s){
			Com::CVector2D d = Com::CVector2D(reclomsh[0], reclomsh[1]) - po_s;
			d.Normalize();
			return d;
		}
		else{
			Com::CVector2D d = Com::CVector2D(reclomsh[(ndiv -1) * 2], reclomsh[(ndiv -1) * 2 + 1]) - po_e;
			d.Normalize();
			return d;
		}
	}
...
Com::CVector2D Cad::CEdge2D::GetNearestPoint(const Com::CVector2D& po_in) const
{
	...
	else if (itype == 3){//  RCJ 6/15/2017 

		const std::vector<double>& relcomsh = aRelCoSpline;
		double min_dist = Com::Distance(po_s, po_in);
		Com::CVector2D nearest = po_s;
		const unsigned int ndiv = relcomsh.size() / 2;

		for (int i = 0; i < ndiv; ++i)
		{
			double len = Com::Distance(Com::CVector2D(relcomsh[i * 2], relcomsh[i * 2 + 1]), po_in);
			if (len < min_dist){
				min_dist = len;
				nearest = Com::CVector2D(relcomsh[i * 2], relcomsh[i * 2 + 1]);
			}
		}
		if (Com::Distance(po_e, po_in) < min_dist){
...
bool Cad::CEdge2D::IsCrossEdgeSelf() const
{
	...
	if (this->itype == 3)return false;  //  RCJ 6/15/2017 
    return false;
}
...
double Cad::CEdge2D::Distance(const CEdge2D& e1) const
{
	...
	//  RCJ 6/15/2017 
	else if (this->itype == 0 && e1.itype == 3){
		double min_dist = -1;
		unsigned int ndiv1 = e1.aRelCoSpline.size() / 2 + 1;
		for (unsigned int idiv = 0; idiv < ndiv1; idiv++){

			Com::CVector2D po0;
			if (idiv == 0){ po0 = e1.po_s; }
			else{ po0 = Com::CVector2D(e1.aRelCoSpline[(idiv - 1) * 2 + 0], e1.aRelCoSpline[(idiv - 1) * 2 + 1]); }
			Com::CVector2D po1;
			if (idiv == ndiv1 - 1){ po1 = e1.po_e; }
			else{ po1 = Com::CVector2D(e1.aRelCoSpline[idiv * 2 + 0], e1.aRelCoSpline[idiv * 2 + 1]); }
			const double dist = GetDist_LineSeg_LineSeg(po_s, po_e, po0, po1);
			if (dist < -0.5)return dist;
			if (dist < min_dist || min_dist < -0.5){ min_dist = dist; }
		}
	...
	else if (this->itype == 3 && e1.itype == 0){
		return e1.Distance(*this);
	}
	else if (this->itype == 3 && e1.itype == 3){

		double min_dist = -1;
		GetDist_LineSeg_LineSeg(po_s, po_e, po_s1, po_e1);
		const std::vector<double>& relcomsh0 = aRelCoSpline;
		const unsigned int ndiv0 = aRelCoSpline.size() / 2+1;
		const std::vector<double>& relcomsh1 = e1.aRelCoSpline;
		const unsigned int ndiv1 = e1.aRelCoSpline.size() / 2+1;
		for (unsigned int idiv = 0; idiv < ndiv0; idiv++){
			Com::CVector2D po0_i;
			if (idiv == 0)po0_i = po_s;
			else{ po0_i = Com::CVector2D(aRelCoSpline[(idiv - 1) * 2 + 0], aRelCoSpline[(idiv - 1) * 2 + 1]); }
			Com::CVector2D po1_i;
			if (idiv == ndiv0 - 1)po1_i = po_e;
			else{ po1_i = Com::CVector2D(aRelCoSpline[idiv * 2], aRelCoSpline[idiv * 2 + 1]); }

			for (unsigned int jdiv = 0; jdiv < ndiv1; ++jdiv){

				Com::CVector2D po0_j;
				if (jdiv == 0){ po0_j = e1.po_s; }
				else{ po0_j = Com::CVector2D(e1.aRelCoSpline[(jdiv - 1) * 2 + 0], e1.aRelCoSpline[(jdiv - 1) * 2 + 1]); }
				Com::CVector2D po1_j;
				if (jdiv == ndiv1 - 1){ po1_j = e1.po_e; }
				else{ po1_j = Com::CVector2D(e1.aRelCoSpline[jdiv * 2 + 0], e1.aRelCoSpline[jdiv * 2 + 1]); }
				const double dist = GetDist_LineSeg_LineSeg(po0_i, po1_i, po0_j, po1_j);
				if (dist < -0.5)return dist;//  RCJ 6/15/2017 
				if (min_dist < -0.5 || dist < min_dist){ min_dist = dist; }
			}
...
bool Cad::CEdge2D::IsCrossEdge_ShareOnePoint(const CEdge2D& e1, bool is_share_s0, bool is_share_s1) const
{
	...
	else if (this->itype == 3){ //  RCJ 6/15/2017 
		return false;
	}
    return false;
}
...
bool Cad::CEdge2D::GetNearestIntersectionPoint_AgainstHalfLine(Com::CVector2D& sec, const Com::CVector2D& org, const Com::CVector2D& dir) const
{
	...
	//  RCJ 6/15/2017 
		...
		for (unsigned int idiv = 0; idiv < ndiv; idiv++){
			Com::CVector2D poi0;
			if (idiv == 0){ poi0 = po_s; }
			else{ poi0 = Com::CVector2D(relcomsh[(idiv - 1) * 2 + 0], relcomsh[(idiv - 1) * 2 + 1]); }

			Com::CVector2D poi1;
			if (idiv == ndiv - 1){ poi1 = po_e; }
			else{ poi1 = Com::CVector2D(relcomsh[idiv * 2 + 0], relcomsh[idiv * 2 + 1]); }
...
double Cad::CEdge2D::GetCurveLength() const
{
	...
	//  RCJ 6/15/2017 
		...
		const unsigned int npo_cad = this->aRelCoSpline.size() / 2;
		double edge_len = 0.0;
		edge_len += sqrt((po_s.x - aRelCoSpline[0])*(po_s.x - aRelCoSpline[0]) + (po_s.y - aRelCoSpline[1])*(po_s.y - aRelCoSpline[1]));
		for (unsigned int idiv = 0; idiv < npo_cad-1; idiv++){
			edge_len += sqrt((aRelCoSpline[idiv * 2 + 0] - aRelCoSpline[(idiv + 1) * 2 + 0])*(aRelCoSpline[idiv * 2 + 0] - aRelCoSpline[(idiv + 1) * 2 + 0])
				+ (aRelCoSpline[idiv * 2 + 1] - aRelCoSpline[(idiv + 1) * 2 + 1])*(aRelCoSpline[idiv * 2 + 1] - aRelCoSpline[(idiv + 1) * 2 + 1]));
		}
		edge_len += sqrt((po_e.x - aRelCoSpline[(npo_cad - 1) * 2 + 0])*(po_e.x - aRelCoSpline[(npo_cad - 1) * 2 + 0]) +
			(po_e.y - aRelCoSpline[(npo_cad - 1) * 2 + 1])*(po_e.y - aRelCoSpline[(npo_cad - 1) * 2 + 1]));
...
bool Cad::CEdge2D::GetCurve_Mesh(std::vector<Com::CVector2D>& aCo, int ndiv) const
{
    ...
    if (ndiv <= 0){    // 尽可能地多而忠实的曲线的表现
        ...
        else if (this->itype == 2) // メッシュの場合
        {
			...
			const unsigned int size = aRelCoSpline.size() / 2;
			for (int i = 0; i < size; ++i){
				Com::CVector2D vec;
				vec.x = aRelCoSpline[i * 2 + 0];
				vec.y = aRelCoSpline[i * 2 + 1];
				aCo.push_back(vec);
			}
...
bool Cad::CEdge2D::ConnectEdge(const Cad::CEdge2D& e1, bool is_add_ahead, bool is_same_dir)
{
    ...
    if (this->itype == 2){
        ...
        {
            const unsigned int ndiv1 = (const unsigned int)(e1.GetCurveLength() / ave_elen);
