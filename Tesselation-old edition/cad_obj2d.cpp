...
////////////////////////////////////////////////////////////////
// CadObj2D.cpp : ２次元ＣＡＤモデルClass(CCadObj2D)の実装
////////////////////////////////////////////////////////////////
...
void CCadObj2D::Set_Edge_InsertIndex(const unsigned int& id_e, const unsigned& id_edge_v_s, const unsigned& id_edge_v_e){

	CEdge2D& edge2d = this->GetEdgeRef(id_e);
	edge2d.id_insert_v_s = id_edge_v_s;
	edge2d.id_insert_v_e = id_edge_v_e;
}
...
bool CCadObj2D::CheckIsPointInside_ItrLoop(CBRepSurface::CItrLoop& itrl, const CVector2D& point) const
{
	...
	//原有的值设置为29  现在将其修改为其他值
	//  RCJ 4/16/2018 
	double val = 29;
	for (unsigned int i = 1; i < val; i++){	// 29 is handy prim number
        unsigned int cross_counter = 0;
        bool iflg = true;
		CVector2D dir(sin(6.28*i / (val*1.0)), cos(6.28*i / (val*1.0)));
        for (itrl.Begin(); !itrl.IsEnd(); itrl++){
            unsigned int id_e;   bool is_same_dir;
            itrl.GetIdEdge(id_e, is_same_dir);
			if (id_e == 0)
...
void CCadObj2D::InsertEdgeAreaLength(int id, double edgelength, double o_area_l, double d_area_l){

	Edge_Area_Length s;
	s.edgelength = edgelength;
	s.ordinary_area_length = o_area_l;
	s.detail_area_length = d_area_l;
	if (m_EdgelengthSet.find(id) == m_EdgelengthSet.end()){
		m_EdgelengthSet.insert(std::make_pair(id, s));
	}
}

double CCadObj2D::Get_Edgelength(int id){
	if (m_EdgelengthSet.find(id) == m_EdgelengthSet.end()){
		return 0.03;
	}
	return m_EdgelengthSet[id].edgelength;
}

double  CCadObj2D::Get_OrdinaryArealength(int id){
	if (m_EdgelengthSet.find(id) == m_EdgelengthSet.end()){
		return 0.03;
	}
	return m_EdgelengthSet[id].ordinary_area_length;
}


double CCadObj2D::Get_DetailArealength(int id){
	if (m_EdgelengthSet.find(id) == m_EdgelengthSet.end()){
		return 0.02;
	}
	return m_EdgelengthSet[id].detail_area_length;
}

void CCadObj2D::ClearLengthset(){
	m_EdgelengthSet.clear();
}
